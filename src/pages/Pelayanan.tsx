  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prevMessages => [
        ...prevMessages.filter(msg => msg.id !== loadingMessageId),
        { 
          text: 'Maaf, terjadi kesalahan saat menghubungi API.', 
          sender: 'bot', 
          id: Date.now() 
        }
      ]);
          ...
          {!isLoggedIn && (
            <div className="p-6 bg-yellow-50 border-b border-yellow-100">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-yellow-800">Anda belum login</h3>
              ...
              >
                ...
                {message.sender === 'response-details' && (
                  <div className="flex items-start space-x-2 max-w-xs md:max-w-md">
                    ...
                    <div className="bg-white rounded-2xl rounded-tl-none py-3 px-4 text-gray-700 shadow-sm">
                      <div>
                        {typeof message.text === 'object' && 'output' in message.text && message.text.output
                          ? message.text.output
                          : typeof message.text === 'object' && 'model_input' in message.text && message.text.model_input
                      ...
                      <div className="mt-3 pt-2 border-t border-gray-200">
                        ...
                        >
                          {openDetailId === message.id ? 'Sembunyikan Detail' : 'Lihat Detail'}
                        </button>
                        
                        {openDetailId === message.id && typeof message.text === 'object' && (
                          <div className="mt-2 bg-gray-50 p-3 rounded text-xs whitespace-pre-wrap border border-gray-100">
                            <div className="mb-1"><strong className="text-purple-600">Original:</strong> {(message.text as ApiResponse).original_question}</div>
                            <div className="mb-1"><strong className="text-purple-600">Translated:</strong> {(message.text as ApiResponse).translated_question}</div>
                            <div><strong className="text-purple-600">Context:</strong> {(message.text as ApiResponse).context}</div>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="p-4 border-t border-gray-200 bg-white">
            <div className="flex rounded-xl border border-gray-300 overflow-hidden focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-transparent">
              <input
                ...
                placeholder={isLoggedIn ? "Ketik pertanyaan Anda..." : "Silakan login untuk menggunakan chatbot"}
              ...
              <button
                ...
                className={`bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 flex items-center justify-center transition-colors ${
                  (isLoading || !inputValue.trim() || !isLoggedIn) ? 'opacity-50 cursor-not-allowed' : 'hover:from-purple-700 hover:to-indigo-700'
                }`}
              >
                {isLoading ? (
            ...
            <div className="mt-2 text-xs text-gray-500 text-center">
              SIPA chatbot akan memberikan informasi terkait layanan perlindungan anak
            </div>
          </div>
        </div>
      </div>
